...
* The KNN classifier selects a response variable based on the response class with the highest conditional probability within ~0~.  
* The KNN regression method evaluates a response variable based on the average of the training responses in N~0~.
...
a) **Suppose that the true relationship between X and Y is linear, i.e. Y = β~0~ + β~1~X + &#1013;. Consider the training residual sum of squares (RSS) for the linear regression, and also the training RSS for the cubic regression. Would we expect one to be lower than the other, would we expect them to be the same, or is there not enough information to tell? Justify your answer.**
